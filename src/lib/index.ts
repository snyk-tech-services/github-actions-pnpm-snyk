...
const runAction = async () => {
  const breakBuild: boolean =
    core.getInput("breakBuild") == "true" ? true : false;
  try {
    ...
    const debug: boolean = core.getInput("debugMode") == "true" ? true : false;
    const showDeps: boolean =
      core.getInput("showDepsInfo") == "true" ? true : false;
    ...
    if (payload.commits && payload.head_commit) {
      ...
      if (fullScan) {
        ...
        const snykTest = execSync(`npx snyk test ${snykArgs} > out || true`, {
          cwd: path,
        });
        if (debug) {
          console.log("================================");
        ...
        switch (result) {
          ...
          case 2: {
            console.log(
              "Error during delta computation - defaulting to full scan"
            );
            // Resets exit code
            process.exitCode = 0;
            ...
            try {
              const snykTest = execSync(`npx snyk test ${snykArgsNormal}`, {
                cwd: path,
              });
              console.log(snykTest.toString());
            } catch (err) {
